...
// ───────────────────────── Live corridor ranking (Road stats) ─────────────────────────

export interface CorridorLive {
  id: string
  name: string
  nameHu: string
  speedKmh: number
  durationSeconds: number
  distanceMeters: number
  delayRatio: number // trafficDelay / freeFlow time
}

export interface CorridorRanking {
  corridors: CorridorLive[] // best (fastest) → worst
  source: ResultSource
}
...
/** Rank a set of corridors by current travel speed using one Matrix call (live or model). */
export async function rankCorridorsLive(
  corridors: CorridorSpec[],
  mode: TravelMode = 'car',
): Promise<CorridorRanking> {
  const origins = corridors.map((c) => c.from)
  const destinations = corridors.map((c) => c.to)
  const grid = await tomtomMatrix(origins, destinations, mode)

  const toLive = (c: CorridorSpec, leg: LegSummary): CorridorLive => {
    const dur = Math.max(leg.durationSeconds, 1)
    const freeFlow = Math.max(dur - leg.trafficDelaySeconds, 1)
    return {
      id: c.id,
      name: c.name,
      nameHu: c.nameHu,
      speedKmh: Math.round((leg.distanceMeters / 1000 / dur) * 3600),
      durationSeconds: leg.durationSeconds,
      distanceMeters: leg.distanceMeters,
      delayRatio: leg.trafficDelaySeconds / freeFlow,
    }
  }

  let source: ResultSource = 'model'
  const live: CorridorLive[] = corridors.map((c, i) => {
    const leg = grid?.[i]?.[i]
    if (leg) {
      source = 'tomtom'
      return toLive(c, leg)
    }
    return toLive(c, modelLeg(c.from, c.to, mode))
  })

  live.sort((a, b) => b.speedKmh - a.speedKmh)
  return { corridors: live, source }
}
